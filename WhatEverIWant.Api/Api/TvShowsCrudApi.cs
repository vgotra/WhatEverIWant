...
public static class TvShowsCrudApi
{
    ...
    public static void MapSeriesApi(this WebApplication app)
    {
        ...
        group.MapGet("/", async (ITvShowsService seriesService) => await seriesService.GetAllAsync().OkAsync())
            .WithEndpointMetadata("GetAllTvShows", "Retrieves all tv shows.")
            .Produces<IEnumerable<TvShowResponse>>();

        group.MapGet("/{id:long}", async (long id, ITvShowsService seriesService) => await seriesService.GetByIdAsync(id).OkOrNotFoundAsync())
            .WithEndpointMetadata("GetTvShowById", "Retrieves a specific tv show by unique id.")
            .Produces<TvShowResponse>()
            .Produces(StatusCodes.Status404NotFound);

        group.MapPost("/", async (CreateTvShowRequest request, ITvShowsService seriesService) => await seriesService.CreateAsync(request).CreatedAsync(x => $"{BaseRoute}/{x.Id}"))
        ...
        group.MapPut("/{id:long}", async (long id, UpdateTvShowRequest request, ITvShowsService seriesService) => await seriesService.UpdateAsync(id, request).NoContentOrNoFoundAsync())
        ...
        group.MapDelete("/{id:long}", async (long id, ITvShowsService seriesService) => await seriesService.DeleteAsync(id).NoContentOrNoFoundAsync())
