...
public static class BooksCrudApi
{
    ...
    public static void MapBooksApi(this WebApplication app)
    {
        ...
        group.MapGet("/", async (IBookService bookService) => await bookService.GetAllAsync().OkAsync())
            .WithEndpointMetadata("GetAllBooks", "Retrieves all books.")
            .Produces<IEnumerable<BookResponse>>();

        group.MapGet("/{id:long}", async (long id, IBookService bookService) => await bookService.GetByIdAsync(id).OkOrNotFoundAsync())
            .WithEndpointMetadata("GetBookById", "Retrieves a specific book by unique id.")
            .Produces<BookResponse>()
            .Produces(StatusCodes.Status404NotFound);

        group.MapPost("/", async (CreateBookRequest request, IBookService bookService) => await bookService.CreateAsync(request).CreatedAsync(x => $"{BaseRoute}/{x.Id}"))
        ...
        group.MapPut("/{id:long}", async (long id, UpdateBookRequest request, IBookService bookService) => await bookService.UpdateAsync(id, request).NoContentOrNoFoundAsync())
        ...
        group.MapDelete("/{id:long}", async (long id, IBookService bookService) => await bookService.DeleteAsync(id).NoContentOrNoFoundAsync())
