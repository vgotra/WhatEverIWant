...
public static class AudioBooksCrudApi
{
    ...
    public static void MapAudioBooksApi(this WebApplication app)
    {
        ...
        group.MapGet("/", async (IAudioBookService audioBookService) => await audioBookService.GetAllAsync().OkAsync())
            .WithEndpointMetadata("GetAllAudioBooks", "Retrieves all audiobooks.")
            .Produces<IEnumerable<AudioBookResponse>>();

        group.MapGet("/{id:long}", async (long id, IAudioBookService audioBookService) => await audioBookService.GetByIdAsync(id).OkOrNotFoundAsync())
            .WithEndpointMetadata("GetAudioBookById", "Retrieves a specific audiobook by unique id.")
            .Produces<AudioBookResponse>()
            .Produces(StatusCodes.Status404NotFound);

        group.MapPost("/", async (CreateAudioBookRequest request, IAudioBookService audioBookService) => await audioBookService.CreateAsync(request).CreatedAsync(x => $"{BaseRoute}/{x.Id}"))
        ...
        group.MapPut("/{id:long}", async (long id, UpdateAudioBookRequest request, IAudioBookService audioBookService) => await audioBookService.UpdateAsync(id, request).NoContentOrNoFoundAsync())
        ...
        group.MapDelete("/{id:long}", async (long id, IAudioBookService audioBookService) => await audioBookService.DeleteAsync(id).NoContentOrNoFoundAsync())
